...
const statusColors: Record<AppointmentStatus, string> = {
  ...
  [AppointmentStatus.COMPLETED]: '#64748b', // Slate
  [AppointmentStatus.CANCELLED]: '#ef4444', // Red
  [AppointmentStatus.ABSENT]: '#f59e0b',    // Orange (New)
};
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
          <Box>
            <Typography variant="h4" component="h1" gutterBottom>Appointments</Typography>
            <Typography variant="body1" color="text.secondary">Manage and view all appointments</Typography>
          </Box>
        </Box>
        ...
        <Paper sx={{ p: 3, mb: 3 }}>
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid size={{ xs: 12, md: 2 }}>
              <Button fullWidth variant="outlined" startIcon={<Refresh />} onClick={loadAppointments} disabled={loading}>
                Refresh
              </Button>
            </Grid>
          </Grid>
        ...
        <Paper sx={{ mb: 3 }}>
          <Tabs value={activeTab} onChange={handleTabChange} variant="scrollable" scrollButtons="auto">
            <Tab label={`All (${appointments.length})`} value="ALL" />
            {Object.values(AppointmentStatus).map(status => (
              <Tab key={status} label={`${statusLabels[status]} (${getAppointmentsByStatus(status).length})`} value={status} />
            ))}
          </Tabs>
        </Paper>
                ...
                <Card>
                  <CardContent>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <Box sx={{ flex: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                          <Typography variant="h6" sx={{ mr: 2 }}>{appointment.metadata?.patientName}</Typography>
                          <Chip
                            label={statusLabels[appointment.status]}
                            size="small"
                            sx={{
                        ...
                        <Grid container spacing={2} sx={{ mt: 1 }}>
                          <Grid size={{ xs: 12, md: 3 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              ...
                              <Typography variant="body2">{format(new Date(appointment.date), 'EEE, MMM dd, yyyy')}</Typography>
                            </Box>
                          </Grid>
                          <Grid size={{ xs: 12, md: 3 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              <AccessTime sx={{ mr: 1, fontSize: 20, color: 'text.secondary' }} />
                              <Typography variant="body2">{appointment.startTime} - {appointment.endTime}</Typography>
                            </Box>
                          </Grid>
                          <Grid size={{ xs: 12, md: 3 }}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              <MedicalServices sx={{ mr: 1, fontSize: 20, color: 'text.secondary' }} />
                              <Typography variant="body2">{appointment.metadata?.serviceName}</Typography>
                            </Box>
                          </Grid>
                        </Grid>
                        {appointment.notes && (
                          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>Notes: {appointment.notes}</Typography>
                        )}
                      </Box>

                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        {/* LOGIC: Doctor can only change status to COMPLETED. 
                            Locked if Cancelled. 
                        */}
                        <Select
                          value={appointment.status}
                        ...
                        >
                          ...
                          <MenuItem value={appointment.status}>{statusLabels[appointment.status]}</MenuItem>

                          {/* Target Status: Only ALLOW "COMPLETED" as a target from other statuses */}
                          {appointment.status !== AppointmentStatus.COMPLETED && appointment.status !== AppointmentStatus.CANCELLED && (
                            <MenuItem value={AppointmentStatus.COMPLETED}>Mark Completed</MenuItem>
                          )}
                        </Select>
                      </Box>
                    </Box>
