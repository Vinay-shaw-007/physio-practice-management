...
const DoctorDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // 2. Upcoming (Future Confirmed) - Replaces "Pending"
      ...
      // 3. Unique Patients
      const uniquePatients = new Set(allAppts.map(a => a.patientId)).size;

      // 4. Revenue (Completed/Confirmed)
      const confirmedRevenue = allAppts
        .filter(a => a.status === AppointmentStatus.COMPLETED || a.status === AppointmentStatus.CONFIRMED)
        .reduce((sum, a) => sum + (a.amount || 0), 0);
  ...
  // Calculate next immediate patient
  const nextAppointment = appointments
    .filter(a => isToday(new Date(a.date)) && a.status !== AppointmentStatus.CANCELLED)
    .find(a => {
      const [h, m] = a.startTime.split(':').map(Number);
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {/* Stats Grid - Using Grid V2 size prop */}
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <StatCard
            title="Appointments Today"
                ...
                {displayList.map((appt) => (
                  <Paper
                    ...
                    sx={{
                      ...
                      transition: 'transform 0.2s',
                      '&:hover': { transform: 'translateY(-2px)', boxShadow: 1 },
                      borderLeft: `4px solid ${appt.status === AppointmentStatus.COMPLETED
